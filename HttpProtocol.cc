...
//        }
      ...
      bool Event_item::CgiHandle(){
        //  std::cout<<"CgiHandle .."<<std::endl;
          ...
          execl(request.path.c_str(),request.path.c_str(),NULL);  
            ...
            int begin1=response_body.find(": ")+2;
            int end1=response_body.find("X");

            std::string is_sendpage=response.response_body.substr(begin1,end1-begin1);
            LOG(INFO,is_sendpage.c_str());
            if(is_sendpage=="true"){
              //发送静态网页
            //Page-index:wwwroot/regisuccess.htmlX
              IsSendPage=true;
              int begin2=response_body.find(": ",end1+1)+2;
              int end2=response_body.find("X",begin2);
              std::string page_index=response_body.substr(begin2,end2-begin2);
              response.fd=open(page_index.c_str(),O_RDONLY);
              struct stat buf;
              stat(page_index.c_str(),&buf);
              response.content_size=buf.st_size;
            }
            else{
              response_body.erase(0,end1+1); 
      ...
      void Event_item::RecvRequest(){
        ...
        //LOG(INFO,"recv line success");
        AnalyRequestline();//解析请求行     
        //LOG(INFO,"analy line success");
        RecvreqestHeader();//读取请求报头
        //LOG(INFO,"recv header success");
        AnalyHeader();//解析请求报头
        //LOG(INFO,"analy header success");
        if(IsrecvContent()){
          RecvContent(); //判断是否需要读取读取请求正文
        }  
        BuildResponse();//构建响应
      }
      ...
      void Event_item::SendResponse(){
        ...
        if(IsSendPage){
          //发送静态网页
          LOG("INFO","SendPage begin");
          SendPage();
          close(response.fd);
        }
        else{
          //发送cgi处理结果
          LOG("INFO","Not SendPage begin");
          SendCgiContent(); 
        }
      }
